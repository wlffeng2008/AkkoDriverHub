...
{
    if (select)
        setStyleSheet("background-color:#F4F4F4; border-radius:32px;");
    else
        setStyleSheet("background-color:#F4F4F4; border-radius:32px;");
...
void simulateMouseClick(QWidget *targetWgt, QPoint pos,
                        Qt::MouseButton button = Qt::LeftButton) {
    QMouseEvent pressEvt(QEvent::MouseButtonPress, pos, button, button,
                         Qt::NoModifier);
    QApplication::sendEvent(targetWgt, &pressEvt);

    QMouseEvent releaseEvt(QEvent::MouseButtonRelease, pos, button, button,
                           Qt::NoModifier);
